...
function showUsers(users) {
  users.forEach((data) => {
    ...
    // create card
    // div (card) --> image(img) --> div (blur) --> div(content)[h2,p (caed-text)]
...
inp.addEventListener("input", () => {
    ...
    let new_user = users.filter ((data) => {
        ...
        return data.name.startsWith(text); // Data mate true aave te 
        // data return (Save) thase

        // search bar --> inp.value.tolowerCase == user.name.tolowerCase
        //method : inp.value.chartAt(0).touppercase + inp.value.slice(1) == user.name
    });
